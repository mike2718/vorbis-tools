...
static flac_picture_t * flac_picture_parse_eat(void *data, size_t len)
{
    ...
    */
    ...
    ret->media_type = (char*)data + 8;
    offset = 8 + tmp;
    tmp = read32be((char*)data + offset);
    expected_length += tmp;
    if (len < expected_length) {
        free(ret);
        return NULL;
    ...
    *(char*)((char*)data + offset) = 0;
    offset += 4;
    ret->description = (char*)data + offset;
    offset += tmp;
    ret->width = read32be((char*)data + offset);
    *(char*)((char*)data + offset) = 0;
    offset += 4;
    ret->height = read32be((char*)data + offset);
    offset += 4;
    ret->depth = read32be((char*)data + offset);
    offset += 4;
    ret->colors = read32be((char*)data + offset);
    offset += 4;
    ret->binary_length = read32be((char*)data + offset);
